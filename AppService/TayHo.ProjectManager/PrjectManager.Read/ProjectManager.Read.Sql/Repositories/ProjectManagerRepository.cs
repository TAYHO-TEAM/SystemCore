...
namespace ProjectManager.Read.Sql.Repositories
{
    public class ProjectManagerRepository : IProjectManagerRepository
    {
        ...
        public async Task<LoadResult> GetAccount(string nameEF, DevLoadOptionsBase dataSourceLoadOptionsBase)
        {

            var config = new MapperConfiguration(cfg =>
                  cfg.CreateMap<DevLoadOptionsBase, DataSourceLoadOptions>()
              );
            ...
            if (objEF != null)
            {
                //List<string> convertFilter = new List<string>();
                //foreach(var item in dataSourceLoadOptions.Filter)
                //{
                //    if(item.GetType().ToString() == "Array")
                //    {
                //        List<string> temp =  ((string[])item).ToList();
                //        convertFilter.AddRange(temp);
                //    }
                //    else
                //    {
                //        convertFilter.Add(item.ToString());
                //    }    
                //}
                //dataSourceLoadOptions.Filter = convertFilter;
                //dataSourceLoadOptions.PrimaryKey = new[] { "ID" };
                //dataSourceLoadOptions.PaginateViaPrimaryKey = true;

                //if (!String.IsNullOrEmpty(dataSourceLoadOptions.Filter[1].ToString()))
                //{
                //    dataSourceLoadOptions.Filter = JsonConvert.DeserializeObject<IList>(Filter, new JsonSerializerSettings
                //    {
