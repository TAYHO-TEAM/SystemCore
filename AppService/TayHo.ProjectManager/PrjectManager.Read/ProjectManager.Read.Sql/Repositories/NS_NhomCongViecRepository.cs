...
namespace ProjectManager.Read.Sql.Repositories
{
    public class NS_NhomCongViecRepository<T> : INS_NhomCongViecRepository<T> where T : class
    {
        ...
        public async Task<PagingItems<T>> GetByGiaiDoan(RequestBaseFilterParam requestBaseFilterParam)
        {
            ...
            using var rs = conn.QueryMultipleAsync("sp_NS_NhomCongViec_GetByGiaiDoan", requestBaseFilterParam, commandType: CommandType.StoredProcedure).Result;
