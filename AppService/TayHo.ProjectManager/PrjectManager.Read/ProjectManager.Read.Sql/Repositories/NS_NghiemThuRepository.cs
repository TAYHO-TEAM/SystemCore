...
{
    public class NS_NghiemThuRepository<T> : INS_NghiemThuRepository<T> where T : class
    {
        protected readonly ISqlConnectionFactory _connectionFactory;

        public NS_NghiemThuRepository(ISqlConnectionFactory connectionFactory)
        {
            _connectionFactory = connectionFactory ?? throw new ArgumentNullException(nameof(connectionFactory));
        }
        public async Task<PagingItems<T>> GetNS_NghiemThuDetailAsync(RequestBaseFilterParam requestBaseFilterParam)
        {
            ...
            using var rs = conn.QueryMultipleAsync("sp_NghiemThu_Detail", requestBaseFilterParam, commandType: CommandType.StoredProcedure).Result;
